# ResourceHelper v.1.4 by Duane Johnson
#
...
module ResourceHelper
  ...
  def require_javascript(*scripts)
    scripts.each do |script|
      require_resource(:javascript, ResourceHelper.path_prefix.to_s + script.to_s)
    end
  end
  ...
  def require_stylesheet(*sheets)
    sheets.each do |sheet|
      require_resource(:stylesheet, ResourceHelper.path_prefix.to_s + sheet.to_s)
    end
  end
  
  # Changes the ResourceHelper.path_prefix within the scope of a block.  This is
  ...
  def require_relative_to(path)
    former_prefix = ResourceHelper.path_prefix
    ResourceHelper.path_prefix = path
    yield
    ResourceHelper.path_prefix = former_prefix
  end
